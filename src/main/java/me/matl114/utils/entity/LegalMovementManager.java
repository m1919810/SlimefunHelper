...
public class LegalMovementManager implements ProgressWrapper<ClientPlayerEntity> {
    ...
    public void tryCorrectMovementInput(){
        ...
        if(yawModified()){
            ...
            //sync values
            input.playerInput = new PlayerInput(forwardSpeed > 0, forwardSpeed < 0, sidewaySpeed > 0, sidewaySpeed < 0, input.playerInput.jump(), input.playerInput.sneak(), input.playerInput.sprint());
        }

...
import me.matl114.utils.impl.inventory.ItemStackSample;
import me.matl114.utils.impl.tasks.LimitedSpeedExecutor;
...
public class InvTasks {
    ...
    public static void quickMoveSlot(ScreenHandler handler, int index, boolean ignoreConfig){
        if(!ignoreConfig &&  handler.getSlot(index).getStack().getCount() > 1 && getFastInv().enableLeftOne.get()){
...
//            realStack = realStack.copy();
    ...
    public static SlotElement.SlotClickCallback getRightClickOpenEditScreenCallback(){
        return (item, button) ->{
            if(button == 1)
                getItemEditor().openEditScreen(item, null);
            return true;
        };
    }
    ...
    public static final Codec<ItemStackDataWithAmount> CUSTOM_AMOUNT_ITEM_DATA_CODEC = ItemStackDataWithAmount.createCodecOf(CUSTOM_ITEM_DATA_CODEC);



    private static void initModules(ModuleManager m){
        invExtra = new InvExtra()
            .register(m);
        fastInv = new FastInv()
            .register(m);
        keepInv = new KeepInv()
            .register(m);
        fastCraft = new FastCraft()
            .register(m);
        fastChest = new FastChest()
            .register(m);

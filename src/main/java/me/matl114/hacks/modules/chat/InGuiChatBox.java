...
public class InGuiChatBox extends BaseModule {
    ...
    public final FlagRef enable = flagBuilder(Configs.CHAT_CONFIG, CHAT_BOX_IN_GUI)
        .build();
    ...
    public void onScreenInitialize(Event<Screen> event) {
        if(event.context instanceof HandledScreenAccess access && enable.get()){
            SubScreenWidget newChat = new ChatLikeInputSubScreen(access.getScreenX() + 2 , access.getScreenY() + access.getScreenBackgroundY() + (access instanceof CreativeInventoryScreen ? 40 : 10), access.getScreenBackgroundX() - 4, 12, (str)-> {

...
public class ChatExtra extends BaseModule {
    ...
    public final FlagRef enableChatScreenTools =
            flagBuilder(Configs.CHAT_CONFIG, ENABLE_CHATSCREEN_TOOLS).build();
    ...
    public void onChatScreenInitialized(Event<Screen> screenEvent) {
        if (screenEvent.context() instanceof ChatScreen chat) {
            ...
            if (enableChatScreenTools.get()) {
                ...
                helperInputField =
                        new TextFieldWidget(mc.textRenderer, chat.width - 250, chat.height - 56, 140, 20, Text.of(""));
                ...
                var but1 = ButtonWidget.builder(Text.literal("save and send"), b -> {
                            ChatTasks.sayMessage(helperInputField.getText(), true);
                        })
                        .dimensions(chat.width - 110, chat.height - 56, 60, 20)
                        .build();
                ...
                access.addDrawableChildTo(ButtonWidget.builder(Text.literal("auto-send"), b -> {
                            chatCache.set(helperInputField.getText());
                            toggle.run();
                        })
                        .dimensions(chat.width - 50, chat.height - 56, 50, 20)
                        .build());
                Runnable toggle2 = TaskManagers.getToggleTask(KEEP_CHAT_INV);
                access.addDrawableChildTo(ButtonWidget.builder(Text.literal("keep-chat-inv"), b -> {
                            toggle2.run();
                        })
                        .dimensions(chat.width - 70, chat.height - 80, 70, 20)
                        .build());
                access.addDrawableChildTo(ButtonWidget.builder(Text.literal("sel-to-unicode"), b -> {
                            if (chat.getFocused() instanceof TextFieldWidget text) {
                                text.setText(ChatUtils.toUnicodedString(text.getText()));
                            }
                        })
                        .dimensions(chat.width - 140, chat.height - 80, 70, 20)
                        .build());
                int2CharInputField =
                        new TextFieldWidget(mc.textRenderer, chat.width - 250, chat.height - 80, 50, 20, Text.of(""));
                ...
                access.addDrawableChildTo(ButtonWidget.builder(Text.literal("int<->char"), b -> {
                            tranlateInt2char(int2CharInputField);
                        })
                        .dimensions(chat.width - 200, chat.height - 80, 50, 20)
                        .build());
                ...
                for (int i = 0; i < len; i++) {
                    ...
                    access.addDrawableChildTo(ButtonWidget.builder(Text.literal(valueOfChar), b -> {
                            ...
                            .dimensions(chat.width - 25 * x, chat.height - 104 - y * 24, 22, 20)
                            .build());
                    if (x >= xm) {
                        x = 0;
                        y += 1;

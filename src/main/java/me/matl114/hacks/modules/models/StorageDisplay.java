...
public class StorageDisplay extends BaseModule {
    ...
    public static ItemStack getRenderingEntityContent(EntityType<?> typed) {
        return spawnEggNewStyleItem.containsKey(typed) ? spawnEggNewStyleItem.get(typed).copy() : null;
    }
    ...
    public static final Set<String> potentialKeys = Set.of(NETWORK_STORAGE_PATH, NETWORK_MOVER_ITEM_PATH, OLD_NETWORK_STORAGE_PATH, NETWORK_BLUEPRINT_PATH, OLD_NETWORK_BLUEPRINT_PATH, LOGITECH_SINGULARITY_PATH, INFINTY_STORAGE_ITEM_PATH, FINALTECH_STORAGE_ITEM_NEW, FINALTECH_STORAGE_ITEM_OLD);
    private static boolean hasAnyStorage(NbtCompound tag){// pass pdc
        return tag != null && (
            // need a slimefun id to keep going
                ...
                tag.getKeys().size() > 1 && tag.getKeys().stream().anyMatch(potentialKeys::contains)
            );
    }

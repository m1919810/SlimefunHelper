...
//    public static final String[] MINE_REACH_HOTKEY = {"hotkeys-toggle", "reach"};
    ...
    public void refreshInteractionRange(double val){
        if(mc.player != null){
            if(lastRange != val){
                ...
                points.sort(Comparator.comparingDouble(v -> v.getX() * v.getX() + v.getY() * v.getY() + v.getZ() * v.getZ()));
                blocksAround = points;
    ...
    public final FlagRef quickMine = toggle(QUICK_MINE_HOTKEY)
        .build();

    public final KeyBindRef quickMineKeyBind = toggleHotkey(QUICK_MINE_HOTKEY, new MultiKeyBind(KeyCode.KEY_LEFT_CONTROL, KeyCode.KEY_M))
        .build();
    ...
    public final EnumRef<Configs.BypassMode> fastBreakBypassMode = builder(Configs.MINE_CONFIG, Configs.BypassMode.class)
    ...
    public final FlagRef enableReach = toggle(REACH_TOGGLE)
        .showConfig()
        .build();

    public final KeyBindRef reachKeybind = toggleHotkey(REACH_TOGGLE, new MultiKeyBind(KeyCode.KEY_LEFT_CONTROL, KeyCode.KEY_R))
        .build();

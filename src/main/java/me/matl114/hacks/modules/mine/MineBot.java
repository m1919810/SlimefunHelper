...
public class MineBot  extends BaseModule {
    ...
    public final FlagRef enable = toggle(MINEBOT_HOTKEYS)
        .build();

    public final KeyBindRef keyBind = toggleHotkey(MINEBOT_HOTKEYS, new MultiKeyBind(KeyCode.KEY_LEFT_CONTROL, KeyCode.KEY_B))
        .build();
    ...
    public void onMineBotTick(){
        ...
        for(int i=0;i< multiplePackets.get();++i){
            int mineResult =  switch (mineBotMode.get()){
    ...
    public int onMineCommon(Supplier<BlockPos> posFinder){
        ...
        do{
            ...
            if(rightClickMode.get()){
                ...
                boolean preCalculation = PlayerInteractionAccess.of(mc.interactionManager).preCalculateInstantBreak(lastMinePos);

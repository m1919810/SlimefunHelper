...
public class TpInteract extends BaseModule {
    ...
    private final float ENABLE_NO_TP_DISTANCE = 1.0f;
    ...
    public void onInteractBlock(Event<PlayerInteractBlockC2SPacket> event) {
        ...
        if (enable.get()) {
            ...
            if (new Box(blockPos).squaredMagnitude(mc.player.getEyePos()) > MathUtils.s2(distance)) {
                if (tpToBlock(blockPos, event.context(), false)) {
                    event.cancel();
                }
            }
        }
    ...
    public void onInteractEntity(Event<PlayerInteractEntityC2SPacket> event) {
        ...
        if (enable.get()) {
            ...
            if (!Objects.equals(((Enum) packet.type.getType()).name(), "ATTACK")) {
    ...
    public void onBlockMine(Event<PlayerActionC2SPacket> event) {
        ...
        if (enable.get()) {
            ...
            double distance = MineTasks.getMineExtra().getReachDistance() + ENABLE_NO_TP_DISTANCE;
            if (new Box(blockPos).squaredMagnitude(mc.player.getEyePos()) > MathUtils.s2(distance)) {
                if (tpToBlock(blockPos, event.context(), true)) {
                    event.cancel();
                }
            }
        }
    ...
    public boolean tpToBlock(BlockPos pos, Packet<?> packetToSend, boolean isMine) {
        // compat Freecam
        ...
        if (selectedPos == null) {
            ...
            if (isMine) {
                ...
                if (useFallMine.get()) {
                    ...
                    if (access.setStartFailBreakPos(pos)) {

...
public class FastCraft extends BaseModule {
    ...
    public final FlagRef enable = flagBuilder(Configs.INV_CONFIG, FAST_CRAFT)
        .build();

    public final FlagRef dropCraft = toggle(TOGGLE_DROP_CRAFT)
        .build();
    ...
    private void addCraftingInventoryButton(CraftingScreen screen){
        ...
        ExecutableWidget toggleLockRecipeButton = ExecutableWidget.instance(95, screen.height /2 - 25, 24, 12)
            .setElementHandler(
                new ButtonElement(
                    TextProvider.of(Text.literal("锁")),
        ...
        ExecutableWidget toggleDropButton = ExecutableWidget.instance(120, screen.height /2 - 72, 24, 12)
            .setElementHandler(
                new ButtonElement(
                    TextProvider.of(Text.literal("喷射")),
    ...
    private void addMerchantInformation(MerchantScreen merchantScreen){
        ...
        access .addDrawableChildTo(new TradeInformationSubScreen(access.getScreenX(), access.getScreenY(),  (MerchantScreen) merchantScreen));
    }
}

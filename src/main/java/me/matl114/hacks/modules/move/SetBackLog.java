...
public class SetBackLog extends BaseModule {
    ...
    public final FlagRef logResync = builder(Configs.MOV_CONFIG
       , Boolean.class)
        .path(MOVE_LOG_RESYNC_PACKETS)
        .defaultValue(false)
        .build();

    public final StringRef logResyncFormat = builder(Configs.MOV_CONFIG
        , String.class)
        .path(MOVE_LOG_RESYNC_FORMAT)
        .defaultValue("&fPos Resync &a[%.2f,%.2f,%.2f]")
        .build();

    public final FlagRef logAc = builder(Configs.MOV_CONFIG
        , Boolean.class)
        .path(MOVE_CHECK_SETBACK)
        .defaultValue(false)
        .build();

    public final StringRef logAcFormat = builder(Configs.MOV_CONFIG
        , String.class)

...
public class ConnectionProxy extends BaseModule {
    ...
    public final FlagRef enable = flagBuilder(Configs.HTTP_CONFIG, HTTP_PROXY_ENABLE)
        .build();
                ...
                switch (proxyType.getValue()){
                    case HttpProxyType.SOCKS ->{
                        if(password == null || password.isEmpty()){
                            ch.addFirst("socks4ClientProxy", new Socks4ProxyHandler(
                                addr, userNo?null :username
                            ));
                        }else {
                            ch.addFirst("socks5ClientProxy", new Socks5ProxyHandler(
                                addr, userNo? null: username, password
                            ));
                        }
                    }
                    ...
                    case HttpProxyType.HTTP ->{
                        ch.addFirst("httpClientProxy", new HttpProxyHandler(
                            addr, userNo? null :username, (password == null || password.isEmpty())?null:password
                        ));
                    }
    ...
    public void onScreenInitialize(Event<Screen> screenEvent){
        if(screenEvent.context() instanceof MultiplayerScreen mp){
            ScreenAccess.of(mp).addDrawableChildTo(ButtonWidget
                .builder(Text.literal("http settings"), b -> Tasks.getConfigSystem().openConfigScreen(Configs.HTTP_CONFIG))
                //.width(70)

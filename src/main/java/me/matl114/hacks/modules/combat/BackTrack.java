...
public class BackTrack extends BaseModule {
    ...
    public final FlagRef enable =flagBuilder(bt.addEnable())
        .build();

    public final KeyBindRef hotkey = moduleEntry(bt.addHotkey(), new MultiKeyBind(), bt.addEnable())
        .build();

    public final DoubleRef maxDelay = doubleBuilder(bt.add("max-delay"))
        .defaultValue(50.0D)
        .build();

    public final FlagRef render = flagBuilder(bt.add("render-old"))
        .build();

    public final DoubleRef maxDistance = doubleBuilder(bt.add("max-distance"))
        .defaultValue(5.0D)
        .build();

    public final FlagRef playerOnly = flagBuilder(bt.add("player-only"))
        .build();

    @Override
    public void registerAll() {
        super.registerAll();
        registerListener(PacketManager.getPacketQueueEvent().getChannel(NetworkSide.CLIENTBOUND), this::onQueuePlayerPosition);
    ...
    public void refreshTarget(){
        if(enable.get()){
            Entity entity = TargetSelector.INSTANCE.searchAttackEntity(maxDistance.get(), true, playerOnly.get() ? (ev) -> ev instanceof PlayerEntity : (ev) -> ev instanceof LivingEntity);
            if(entity != currentTarget){
                ...
                }else if(!TargetSelector.INSTANCE.isWithinAttackRange(mc.player.getPos(), currentTarget.getBoundingBox(), CombatExtra.INSTANCE.getAttackAtTargetRange(currentTarget))) {
                    setNoDelay();
                }
            }
        }else {
    ...
    public void onQueuePlayerPosition(Event<PacketStorage> event){
        if(enable.get() && currentTarget != null){
            ...
            if(storage instanceof PacketManager.PacketStorageImpl impl){
                ...
                if(packet instanceof EntityPositionSyncS2CPacket positionSync && positionSync.id() == currentTarget.getId()){
                    onShutdown();
                    return;
                }
                if(packet instanceof EntityPositionS2CPacket position && position.entityId() == currentTarget.getId()){
                    onShutdown();
                    return;
                }
                if(packet instanceof EntityS2CPacket entityMove && entityMove.getEntity(mc.world) == currentTarget && entityMove.isPositionChanged()){
                    ...
                    vec3d = trackedPosition.withDelta((long)entityMove.getDeltaX(), (long)entityMove.getDeltaY(), (long)entityMove.getDeltaZ());
                ...
                if(packet instanceof EntityStatusS2CPacket entityStatus && entityStatus.getStatus() == EntityStatuses.USE_TOTEM_OF_UNDYING && entityStatus.getEntity(mc.world) == mc.player){
                    setNoDelay();
                    flushAll();
                    return;
                }
    ...
    public void handleTrackEntityPosition(Vec3d position){
        if(lastTrackingPosition != null){
            ...
            boolean currentCanAttack = TargetSelector.INSTANCE.isWithinAttackRange(mc.player.getPos(), currentBox, attackRange);
            boolean futureCanAttack = TargetSelector.INSTANCE.isWithinAttackRange(mc.player.getPos(), futureBox, attackRange);
    ...
    public void onRender(Event<MatrixStack> eventMatrixStack){
        if(render.get() && shouldDelay && lastTrackingPosition != null && currentTarget != null){
            ...
            try{
                RenderUtils .drawOutlinedBox(eventMatrixStack.context, boundingBox.getMinPos(), boundingBox.getMaxPos(), Color.ORANGE);
            }finally {

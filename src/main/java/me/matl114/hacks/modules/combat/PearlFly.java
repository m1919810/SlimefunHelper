...
public class PearlFly extends BaseModule {
    ...
    public final FlagRef enable = flagBuilder(pearl.addEnable())
        .build();

    public final KeyBindRef hotkey = moduleEntry(pearl.addHotkey(), new MultiKeyBind(), pearl.addEnable())
        .build();

    public final FlagRef enableCrawl = flagBuilder(pearl.add("enable-crawl"))
        .build();

    public final FlagRef enableStand = flagBuilder(pearl.add("enable-stand"))
        .build();

    public final FlagRef autoCrawl = flagBuilder(pearl.add("auto-crawl"))
        .build();

    public final FlagRef offhand = flagBuilder(pearl.add("offhand"))
        .build();
        ...
        if(enable.get()){
            ...
            do{
                ...
                if(MathUtils.isInXZRange(ppos, searchPos, 0.5 + 0.31)){
                    BlockState state = mc.world.getBlockState(mc.player.getBlockPos().offset(search));
                    ...
                    if(autoCrawl.get() && pose != EntityPose.SWIMMING){
                        BlockState state2 = mc.world.getBlockState(mc.player.getBlockPos().offset(search).offset(Direction.UP));
    ...
    public boolean usePearl(Vec3d look){
        ...
        Runnable runnable = offHand ? InvExtra.INSTANCE.swapInventoryIndexToOffhand(re.index()) : InvExtra.INSTANCE.swapInventoryIndexToHand(re.index());

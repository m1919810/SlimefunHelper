...
public class TargetSelector extends BaseModule {
    ...
    public final DoubleRef multiplyBackward = builder(Configs.COMBAT_CONFIG, COMBAT_OPPOSITE_ATTACK_MULTIPLY, DoubleRef.TYPE)
        .defaultValue(114514.0D)
        .build();

    public final DoubleRef multiplyPlayer = builder(Configs.COMBAT_CONFIG, COMBAT_PLAYER_ATTACK_MULTIPLY, DoubleRef.TYPE)
        .defaultValue(0.0D)
        .build();
...
//                    handleShieldPredict(mc.player.getPitch(), mc.player.getYaw());
        ...
        if(!autoSelect && mc.crosshairTarget.getType()== HitResult.Type.BLOCK && CombatTasks.notSuitableForAttack( mc.player.getMainHandStack())){
    ...
    public Entity searchAimableEntity(boolean commonBow){
        ...
        HitResult result = RaycastUtils.createEntityOnlyCrossHairResult(mc.player, 25, 1.0F, entityCheck -> canAttack(entityCheck) && checkWeapon(entityCheck, commonBow));
        if(result != null && result.getType() == HitResult.Type.ENTITY){
            //focusing entity， attack
            //should respect whitelist
            return ((EntityHitResult)result).getEntity();
    ...
    private double withMultiply(Entity e, double v){
        return Math.abs(v) - ((v < 0.0)? multiplyBackward.get(): 0.0D) + (e instanceof PlayerEntity ? multiplyPlayer.get() : 0.0D);
    }

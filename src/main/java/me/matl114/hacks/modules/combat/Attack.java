...
public class Attack extends BaseModule {
    ...
    public final FlagRef enable = toggle(HOTKEY_ATTACK)
        .build();

    public final KeyBindRef hotkey = toggleHotkey(HOTKEY_ATTACK, new MultiKeyBind(KeyCode.KEY_LEFT_CONTROL, KeyCode.KEY_K))
        .build();

    public final FlagRef legalMode = flagBuilder(Configs.COMBAT_CONFIG, COMBAT_LEGAL_MOD)
        .build();

    public final FlagRef enableTp = flagBuilder(Configs.COMBAT_CONFIG, COMBAT_TP_ENABLE)
        .build();
    ...
    public final FlagRef enableMace = flagBuilder(Configs.COMBAT_CONFIG, COMBAT_MACE_ENABLE)
        .build();
    ...
    public final FlagRef exactAttack = flagBuilder(Configs.COMBAT_CONFIG, COMBAT_EXACT_ATTACK)
        .build();

    public final EnumRef<Configs.LegalTargetingMode> legalTargetingMode = builder(Configs.COMBAT_CONFIG, COMBAT_LEGAL_TARGETTING, Configs.LegalTargetingMode.class)
        .defaultValue(Configs.LegalTargetingMode.DELAY_MOVEMENT)
        .build();

    @ApiStatus.Experimental
    public final FlagRef critic = flagBuilder(Configs.COMBAT_CONFIG, COMBAT_CRITIC)
        .build();


    public final FlagRef renderAttackTarget = flagBuilder(Configs.COMBAT_CONFIG, COMBAT_RENDER_TARGET)
        .build();
    ...
    private boolean processLegalAttack(Entity target){
        ...
        if(mc.crosshairTarget instanceof EntityHitResult entity && entity.getEntity() == target){
            ...
            boolean useTp = (canUseTp() && target.getBoundingBox().squaredMagnitude(mc.player.getEyePos()) > MathUtils.s2(attackRange));
            boolean delayTurningAround = legalTargetingMode.getValue() == Configs.LegalTargetingMode.DELAY_MOVEMENT || useTp ;
                ...
                ClientPlayerAccess.of(mc.player).getLegalMovementManager().addMovementModifier(new LegalMovementManager.MovementModifier() {
                    ...
                    public void applyPreTickModify(Event<LegalMovementManager> movementManagerEvent) {
                        ...
                        if(tpRange.get()> 1E-7 && target.getBoundingBox().squaredMagnitude(mc.player.getEyePos()) > MathUtils.s2(attackRange)){
                            ...
                            Vec3d vec3d1 = MovTasks.tpAttackSearch(vec3d, target.getBoundingBox(), attackRange, 9.9, 1).stream().findFirst().orElse(null);
                            ...
                            //backoff
                            if(target.getBoundingBox().squaredMagnitude(mc.player.getEyePos()) > MathUtils.s2(attackRange)){
                        ...
                        if(distancePassAttack){
                            ...
                            EntityUtils.setEntityRotationSafe(args, cacheDirection);
                            if(canUseMaceTp() && mc.player.getActiveItem().getItem() instanceof MaceItem){
                                Debug.chat(Text.literal("[Attack Bot] Mace Attack Simulation is not supported in legal mode !"));
                            }
                        }
...
//                }
    ...
    private boolean processMaceAttack(PlayerEntity player, Entity target, Deque<MovTasks.MovInfo> movementStack, Deque<MovTasks.MovInfo> shouldMoveBackStack){
...
//        }
    ...
    private static boolean processVanillaAttack(PlayerEntity player, Entity target, Deque<MovTasks.MovInfo> movementStack, Deque<MovTasks.MovInfo> shouldMoveBackStack , boolean alreadAtTarget){
        ...
        if(alreadAtTarget){
            return true;
        }else if(target.getBoundingBox().squaredMagnitude(top.add(0, mc.player.getStandingEyeHeight(), 0)) <= MathUtils.s2(CombatTasks.getCombatExtra().getAttackRange())){
            return true;
        }else return false;
    }
    private boolean processExactAttack(PlayerEntity player, Entity target, Deque<MovTasks.MovInfo> movementStack, Deque<MovTasks.MovInfo> shouldMoveBackStack, boolean vanillaSuccess){
...
//                }
    ...
    private boolean processCommonTpAttack(PlayerEntity player, Entity target, Deque<MovTasks.MovInfo> movementStack, Deque<MovTasks.MovInfo> shouldMoveBackStack, boolean alreadyAtTarget){

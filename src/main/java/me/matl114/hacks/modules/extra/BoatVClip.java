...
public class BoatVClip extends BaseModule implements LegalMovementManager.MovementModifier {
    ...
    public final FlagRef enable = flagBuilder(boatVClip.addEnable())
        .build();
...
//                        Listener.sendPacketNoEvents(new PlayerActionC2SPacket(PlayerActionC2SPacket.Action.START_DESTROY_BLOCK, pos, Direction.UP, NetworkUtils.generateNextSequence()));
//                        Listener.sendPacketNoEvents(new PlayerActionC2SPacket(
//                            PlayerActionC2SPacket.Action.STOP_DESTROY_BLOCK, pos, Direction.UP, NetworkUtils.generateNextSequence()));
//                    }, 0);
//                }
                update.cancel();
            }
    ...
    public void applyPreTickModify(Event<LegalMovementManager> movementManagerEvent) {
        if(enable.get()){
            if(lastVehicle == null || lastVehicle.isRemoved() || lastVehicle.getBoundingBox().squaredMagnitude(mc.player.getEyePos()) > CombatExtra.INSTANCE.getAttackRange()){
                lastVehicle = null;
            }
            if(lastVehicle == null){
                lastVehicle = mc.world.getOtherEntities(mc.player, mc.player.getBoundingBox().expand(1.5, 1.5,1.5)).stream().filter(s -> s instanceof VehicleEntity).findAny().orElse(null);
            }
            if(lastVehicle != null){
                if(!mc.player.hasVehicle()){
...
//                    }
                    ...
                    if(cd > 4){
                        ...
                        var re = new EntityHitResult(lastVehicle, boxxx.getCenter().add(0, boxxx.getLengthY() / 2,0));

...
public class PacketDebugger extends BaseModule {
    public static final String[] DEBUG_PACKET_IN = new String[] {
        "packet-debugger", "debug-packet-in"
    };

    public static final String[] DEBUG_PACKET_OUT = new String[] {
        "packet-debugger", "debug-packet-out"
    };

    public static final String[] DEBUG_PACKET_TYPE = new String[] {
        "packet-debugger", "debug-packet-type"
    };

    public static final String[] INTERCEPT_PACKET = new String[] {
        "packet-debugger", "intercept-packet"
    };

    public static final String[] INTERCEPT_PACKET_TYPE = new String[] {
        "packet-debugger", "intercept-packet-type"
    };
    ...
    public final FlagRef debugIn = flagBuilder(Configs.TEST_CONFIG, DEBUG_PACKET_IN)
        .build();

    public final FlagRef debugOut = flagBuilder(Configs.TEST_CONFIG, DEBUG_PACKET_OUT)
        .build();
    ...
    public final FlagRef interceptPacket = flagBuilder(Configs.TEST_CONFIG, INTERCEPT_PACKET)
        .build();
        ...
        if(debugIn.get()){
            ...
            if(typesDebug.contains(type.getPacketId())){
                if(type instanceof PlayerPositionLookS2CPacket positionLookS2CPacket){
                    Vec3d vec3d = new Vec3d(positionLookS2CPacket.getX(), positionLookS2CPacket.getY(), positionLookS2CPacket.getZ());
                    ExtraTasks.debug("Accept", type.getPacketId().id(), vec3d.x, vec3d.y, vec3d.z, ", Pitch:", positionLookS2CPacket.getPitch(), ", Yaw:", positionLookS2CPacket.getYaw());
                } else{
        ...
        if(debugOut.get()){
            ...
            if(typesDebug.contains(type.getPacketId())){
                if(type instanceof PlayerMoveC2SPacket moveC2SPacket){
                    ExtraTasks.debug("Send", type.getPacketId().id(), moveC2SPacket.getX(0.0), moveC2SPacket.getY(0.0), moveC2SPacket.getZ(0.0),", Pitch:", moveC2SPacket.getPitch(0.0F), ", Yaw:",moveC2SPacket.getYaw(0.0F));
                } else{

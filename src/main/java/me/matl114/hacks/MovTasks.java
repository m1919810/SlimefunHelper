...
import me.matl114.utils.impl.entity.EntityMovementStatus;
...
//        }
    ...
    private static void configureTpMaskPlayer(Event<ClientPlayerEntity> plaayer){
        ...
        ClientPlayerAccess.of(plaayer.context).getLegalMovementManager().addMovementModifier(
...
//        );
    ...
    public static Vec3d collideWithTrustedList(Box currBoundingBox, Vec3d movement, List<VoxelShape> potentialCollisionsVoxel, List<Box> potentialCollisionsBB, double stepHeight, boolean onGround){
        ...
        if (!getMovExtra().noStepHeightFeature.get() && stepHeight > 0.0
...
//    }
    ...
    private static void initModules(ModuleManager m){
        //move
        movExtra = new MovExtra()
            .register(m);
        forwardTp = new ForwardTp()
            .register(m);
        noSlowDown = new NoSlowDown()
            .register(m);
        noFall = new NoFall()
            .register(m);
        ...
        moveTimer = new MoveTimer()
            .register(m);
        stepHeight = new StepHeight()
            .register(m);
        elytraExtra = new ElytraExtra()
            .register(m);
    }
    static{
        //basic structure
...
//        Listener.registerSinglePacketListener(PlayerPositionLookS2CPacket.class, MovTasks::listenPositionResync);
        ...
        Listener.getPacketPreHandlePoint().getChannel(PlayerPositionLookS2CPacket.class).registerHandler(MovTasks::fixPositionSetBackFallDamage);

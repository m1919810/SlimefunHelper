...
public class ACPostTasks {
    ...
    public static void addPostTransactionAction(Consumer<ClientPlayNetworkHandler> packet){
        if(!SlimefunHelper.HACK_VERSION || lastPingTick < Tasks.getTick() - 10){
...
//        Debug.info("in", packetPing.getPacketId(), peekPingRequest);
    ...
    public static void postPongPacketOut(Event<Packet<?>> event){
        //we sent the Common Pong in Ping's handle
        if(event.context() instanceof CommonPingS2CPacket pong){
            //end transaction,
            //fresh queue
            peekPingRequest -= 1;
//            Debug.info("out",pong.getPacketId(), peekPingRequest);
    ...
    static{
        ...
        Listener.getMainThreadPacketPostApplyPoint().registerHandler(ACPostTasks::postPongPacketOut);

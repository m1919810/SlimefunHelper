...
public class ItemStackHelper {
    ...
    public static Map<String, Object> saveItemToMap(Object itemStack) {
        ...
        return (Map<String, Object>) ItemStack.CODEC.encodeStart(ItemStackUtils.registry().getOps(JavaOps.INSTANCE), itemStack1).getOrThrow();
    }
    ...
    public static ItemStack loadItemFromMap(Map<String, Object> itemStack) {
        ...
        return ItemStack.CODEC.decode(ItemStackUtils.registry().getOps(JavaOps.INSTANCE), itemStack).getOrThrow().getFirst();
    }
    ...
    public static List<String> getLore(Object what) {
        ...
        return texts.isEmpty() ? null : texts.stream().map(ChatUtils::textToLegacyString).collect(ArrayList::new, ArrayList::add, ArrayList::addAll);
    }
    ...
    public static void setLore(Object what, List<String> lore) {
        ...
        ItemStackUtils.setLore(itemStack, (lore == null || lore.isEmpty()) ? null : lore.stream().map(ChatUtils::textFromLegacyString).map(Text.class::cast).toList());
    }
    ...
    public static Inventory createInventory(List<?> list, int size){
        return new MutableInventory(size, list.stream().map(s -> JsHelper.unwrap(s, ItemStack.class)).collect(Collectors.toCollection(ArrayList::new)));
    }
    ...
    public static Inventory createJSMappingInventory(List<?> list, int size){
        List<xyz.wagyourtail.jsmacros.client.api.helpers.inventory.ItemStackHelper> helpers = (List<xyz.wagyourtail.jsmacros.client.api.helpers.inventory.ItemStackHelper>) list;
        ...
        return new Inventory() {
            ...
            public boolean isEmpty() {
                return helpers.stream().allMatch(xyz.wagyourtail.jsmacros.client.api.helpers.inventory.ItemStackHelper::isEmpty);
            }
            ...
            public ItemStack removeStack(int slot) {
                ...
                if(itemStack.isEmpty()){
                    ...
                    helpers.set(slot, new xyz.wagyourtail.jsmacros.client.api.helpers.inventory.ItemStackHelper(ItemStack.EMPTY));
                    return itemStack;
                }
            }

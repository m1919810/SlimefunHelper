...
public class SavedItemWidget extends SubScreenWidget{
    ...
    private static final List<Text> COPYCOMMAND_TOOLTIPS = List.of(
        Text.literal("拷贝获取/give指令")

    );
    private static final List<Text> OPENEDITOR_TOOLTIPS = List.of(
        Text.literal("在物品编辑器中打开该物品")
    );
    private static final List<Text> DELITEM_TOOLTIPS = List.of(
        Text.literal("将该物品移除保存物品")
    );
    private final void init0(){
        ...
        ExecutableWidget.instance(75, 36,  25,  16)
            .setElementHandler(
                new ButtonElement(TextProvider.of(Text.literal("Give")), ButtonAction.run(()->{
        ...
        ExecutableWidget.instance(105, 36,  25,  16)
            .setElementHandler(
                new ButtonElement(TextProvider.of(Text.literal("Command")), ButtonAction.run(()->{

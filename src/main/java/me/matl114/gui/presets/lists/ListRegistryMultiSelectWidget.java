...
public class ListRegistryMultiSelectWidget<T> extends ListMultiSelectWidget<Triplet<String, Identifier, T>> {
    ...
    private static <T>
            Pair<List<Triplet<String, Identifier, T>>, Set<Triplet<String, Identifier, T>>> buildPairInternal(
                    Registry<T> registry, Set<T> currentSelection, Function<T, String> localization) {
    ...
    public ListRegistryMultiSelectWidget(
            ...
            Function<T, String> localization,
            BiFunction<Triplet<String, Identifier, T>, AttrKeyValue<Boolean>, RenderHandler> renderFactory,
            int x,
            int y,
            int dx,
            int dy,
    ...
    private ListRegistryMultiSelectWidget(
            Pair<List<Triplet<String, Identifier, T>>, Set<Triplet<String, Identifier, T>>> pairData,
            BiFunction<Triplet<String, Identifier, T>, AttrKeyValue<Boolean>, RenderHandler> renderFactory,
            int x,
            int y,
            int dx,
    ...
    public static ListRegistryMultiSelectWidget<Item> item(
            Set<Item> currentSelect, int x, int y, int dx, int dy, int height) {
        return new ListRegistryMultiSelectWidget<>(
                Registries.ITEM,
                currentSelect,
                (item -> item.getName().getString()),
                (trp, attr) -> new RegistryDisplayRender(
                        new ItemStack(trp.getC()), trp.getC().getName(), trp.getB()),
                x,
                y,
                dx,
                dy,
    ...
    public static ListRegistryMultiSelectWidget<Enchantment> enchant(
            Set<Enchantment> currentSelect, int x, int y, int dx, int dy, int height) {
        return new ListRegistryMultiSelectWidget<>(
                ItemStackUtils.registry().getOptional(RegistryKeys.ENCHANTMENT).orElseThrow(),
                currentSelect,
                (item -> item.description().getString()),
                (trp, attr) -> new RegistryDisplayRender(
                        new ItemStack(Items.ENCHANTED_BOOK), trp.getC().description(), trp.getB()),
                x,
                y,
                dx,
                dy,
    ...
    public static ListRegistryMultiSelectWidget<EntityAttribute> attribute(
            Set<EntityAttribute> currentSelect, int x, int y, int dx, int dy, int height) {
        return new ListRegistryMultiSelectWidget<>(
                Registries.ATTRIBUTE,
                currentSelect,
                (item -> Text.translatable(item.getTranslationKey()).getString()),
                (trp, attr) -> new RegistryDisplayRender(
                        new ItemStack(Items.ANVIL), Text.translatable(trp.getC().getTranslationKey()), trp.getB()),
                x,
                y,
                dx,
                dy,
    ...
    public static <T> ListRegistryMultiSelectWidget<T> registry(
        ...
        if (registry == Registries.ITEM) {
            ...
            return new ListRegistryMultiSelectWidget<>(
                    ...
                    (Object::toString),
                    (trp, attr) ->
                            new RegistryDisplayRender(new ItemStack(Items.AIR), Text.literal(trp.getA()), trp.getB()),
                    x,
                    y,
                    dx,
                    dy,

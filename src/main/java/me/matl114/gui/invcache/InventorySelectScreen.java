...
public class InventorySelectScreen extends GenericBackGroundScreen {
    ...
    private static final List<Text> RULE_ACCEPT_VIRTUAL = List.of(
      Text.literal(  "点击切换容器过滤规则"),
        Text.empty(),
       Text.literal( "当前过滤规则: 接受虚拟容器(即不存在实体方块的容器)")
    );
    private static final List<Text> RULE_REJECT_VIRTUAL = List.of(
        Text.literal(  "点击切换容器过滤规则"),
        Text.empty(),
        Text.literal( "当前过滤规则: 拒绝虚拟容器(即不存在实体方块的容器)")
    );
    ...
    protected DrawableWidget makeIcon(HandledScreen<?> screen){
        ...
        if(screen instanceof TileInventoryScreen tile && !tile.isVirtual()){
            ...
            description.add(Text.literal("记录世界: " ).append(Text.literal(tile.getWorld().getRegistryKey().getValue().toString())));
        }else {
            description.add(Text.literal("虚拟容器").formatted(Formatting.YELLOW));
        }
        return ExecutableWidget.instance(0,0,16,16)
            .setElementHandler(
                SlotElement.instance(

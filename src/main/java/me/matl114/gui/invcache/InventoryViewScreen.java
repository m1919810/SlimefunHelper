...
//    protected HandledScreen<?> handledScreen;
    ...
    private static List<Slot> streamInventoryToSlot(Inventory inventory){
        return IntStream.range(0, inventory.size()).mapToObj((i)->new Slot(inventory, i, 0,0)).toList();
    }
    ...
    protected static List<Text> CLICK_ITEM_TOOLTIPS = List.of(
        Text.literal("左键物品栏中的物品执行选中后操作(可能不存在选中后操作)"),
        Text.literal("右键物品栏中的物品以打开物品编辑界面")
    );
        ...
        super.init();
         ...
         if(blockPos !=null){
             labelElement = LabelElement.instance(
                 Text.literal("%s [%d, %d, %d] (鼠标悬浮以查看说明)".formatted(this.blockWorld.getRegistryKey().getValue().toString(), blockPos.getX(), blockPos.getY(), blockPos.getZ()))
             )
                 ...
                 .withTooltips(TooltipHandler.of(Streams.concat(CLICK_ITEM_TOOLTIPS.stream(), CLICK_COPY_TOOLTIPS.stream()).toList()));
         }else {
             //todo: 增加说明
             labelElement = LabelElement.instance(
                 Text.literal("虚拟容器(鼠标悬浮以查看说明)")
             )
    ...
    protected DrawableWidget makeIcon(Slot screen){
        return ExecutableWidget.instance(0,0,16, 16)
            .setElementHandler(
                new SlotElement(screen.inventory, screen.getIndex(), (stack, i)->{
                    if(i == 1){

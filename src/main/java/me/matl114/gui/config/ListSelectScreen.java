...
public class ListSelectScreen<W> extends ScrollableListWidget {
    ...
    public ListSelectScreen(List<W> lst, Function<W, RenderHandler> renderFactory, Predicate<W> filter, int x, int y, int dx, int dy, int height){
        super(x, y, dx, dy);
        this.list = lst;
    ...
    protected void init(){
        ...
        this.scrollableBorder
            .addDrawableChild(
                McWidgetHelpers.createTextFieldEditBox(1, -textHeight +1, dx -2, textHeight-2, PropertyTracker.event(this::applyFilter), FilterService.currentUserInput)
            );

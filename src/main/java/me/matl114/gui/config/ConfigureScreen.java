...
//                            .setElementHandler(new ButtonElement(TextProvider.of(Text.literal(entry.getKey())), ButtonAction.run(this::saveEntryToValues)))
////
////                    ButtonWidget
////                    .builder(Text.literal(entry.getKey()), b ->saveEntryToValues())
////                    .dimensions(10 , 0 , buttonWidth, buttonHeight).build()
//                );
//            var textField = new TextFieldWidget(this.textRenderer, buttonWidth + 20, 0, buttonWidth, buttonHeight, Text.empty());
//
//            // 设置一些属性
...
//        }).toList();
//        addDrawableChild(new ListUnmodifiableWidget(ListEntryWidgetController.immutable(drawableWidgets, Function.identity(), buttonHeight, 20 + 2 * buttonWidth), 10, 10, this.width - 40, this.height - 20));
      //  addDrawableChild()

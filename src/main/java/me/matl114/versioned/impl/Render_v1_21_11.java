...
public class Render_v1_21_11 implements VRender {
    ...
    public void drawLineVirtual(MatrixStack matrixStack, List<Vec3d> path, Color color) {
        ...
        for(var  i = 1 ; i < points.size() ; i+= 2) {
            ...
            float t = prev.negate().dot(normal);
            float length = next.sub(prev).length();
            if(t > 0 && t < length)
            {
                Vector3f closeToCam = new Vector3f(normal).mul(t).add(prev);
                consumer.vertex(entry, closeToCam).color(color.getRed(), color.getGreen(), color.getBlue(), 255)
                    .normal(entry, normal).lineWidth(2);
                consumer.vertex(entry, closeToCam).color(color.getRed(), color.getGreen(), color.getBlue(), 255)
                    .normal(entry, normal).lineWidth(2);
            }
    ...
    public void drawOutlinedBoxCameraCoord(Matrix4f matrix, Vec3d from, Vec3d to) {
        VertexConsumerProvider.Immediate vcp = getVCP();
        VertexConsumer consumer = vcp.getBuffer(LINES);
        drawOutlinedBox(consumer, from, to);
        vcp.draw(LINES);
    }

    public void drawOutlinedBox(VertexConsumer bufferBuilder, Vec3d from, Vec3d to){
    ...
    public void drawSolidBox(Matrix4f matrix, VertexConsumer bufferBuilder, Vec3d from, Vec3d to) {
        ...
        bufferBuilder.vertex(matrix, minX, minY, minZ).color(cacheRenderColor[0], cacheRenderColor[1], cacheRenderColor[2], cacheRenderColor[3]);
        bufferBuilder.vertex(matrix, maxX, minY, minZ).color(cacheRenderColor[0], cacheRenderColor[1], cacheRenderColor[2], cacheRenderColor[3]);
        bufferBuilder.vertex(matrix, maxX, minY, maxZ).color(cacheRenderColor[0], cacheRenderColor[1], cacheRenderColor[2], cacheRenderColor[3]);
        bufferBuilder.vertex(matrix, minX, minY, maxZ).color(cacheRenderColor[0], cacheRenderColor[1], cacheRenderColor[2], cacheRenderColor[3]);

        bufferBuilder.vertex(matrix, minX, maxY, minZ).color(cacheRenderColor[0], cacheRenderColor[1], cacheRenderColor[2], cacheRenderColor[3]);
        bufferBuilder.vertex(matrix, minX, maxY, maxZ).color(cacheRenderColor[0], cacheRenderColor[1], cacheRenderColor[2], cacheRenderColor[3]);
        bufferBuilder.vertex(matrix, maxX, maxY, maxZ).color(cacheRenderColor[0], cacheRenderColor[1], cacheRenderColor[2], cacheRenderColor[3]);
        bufferBuilder.vertex(matrix, maxX, maxY, minZ).color(cacheRenderColor[0], cacheRenderColor[1], cacheRenderColor[2], cacheRenderColor[3]);
    ...
    public void drawQuadCameraCoord(Matrix4f matrix4f, Vec3d a, Vec3d b, Vec3d c, Vec3d d) {
        ...
        bufferBuilder.vertex(matrix4f, (float) a.x, (float) a.y, (float) a.z).color(cacheRenderColor[0], cacheRenderColor[1], cacheRenderColor[2], cacheRenderColor[3]);
        bufferBuilder.vertex(matrix4f, (float) b.x, (float) b.y, (float) b.z).color(cacheRenderColor[0], cacheRenderColor[1], cacheRenderColor[2], cacheRenderColor[3]);
        bufferBuilder.vertex(matrix4f, (float) c.x, (float) c.y, (float) c.z).color(cacheRenderColor[0], cacheRenderColor[1], cacheRenderColor[2], cacheRenderColor[3]);
        bufferBuilder.vertex(matrix4f, (float) d.x, (float) d.y, (float) d.z).color(cacheRenderColor[0], cacheRenderColor[1], cacheRenderColor[2], cacheRenderColor[3]);
        vcp.draw(QUADS);
    }

...
public class DrawContext_v1_21_11 implements VDrawContext {
    ...
    public void drawGuiTexture(Identifier texture, int i, int j, int k, int l, int x, int y, int z, int width, int height) {
        this.drawContext.drawGuiTexture(RenderPipelines.GUI_TEXTURED, texture, i, j, k, l, x, y, width, height, getShaderRGB());
    }

    @Override
    public void drawTexturedQuad(Identifier texture, int x1, int x2, int y1, int y2, int z, float u1, float u2, float v1, float v2) {
        this.drawContext.drawTexturedQuad(RenderPipelines.GUI_TEXTURED, texture, x1, x2, y1, y2, u1, u2, v1, v2, getShaderRGB());
    }
    ...
    public void drawTooltip(TextRenderer textRenderer, List<Text> text, Optional<TooltipData> data, int x, int y) {
        ...
        if (!list.isEmpty()) {
            addInternal( () -> {
                drawContext.drawTooltipImmediately(textRenderer, list, (int) point1.x, (int) point1.y, HoveredTooltipPositioner.INSTANCE, null);
            });
        }
    }

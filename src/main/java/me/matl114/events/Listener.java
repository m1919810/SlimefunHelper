...
//    }
    ...
    private static Packet<?> unpackMultiPacket(ClientConnection connection,Packet<?> packet,boolean isS2C ) {
        if(packet instanceof BundleS2CPacket bundle){
            ...
            for (var pkt:iter){
                Packet<? super ClientPlayPacketListener> p = (Packet<? super ClientPlayPacketListener>) unpackMultiPacket(connection, pkt, isS2C);
                if(p != null){
                    packets.add(p);
                    if(p != pkt){
                        recreate = true;
    ...
    @Getter
    @Broadcast
    @ExtraArgs(value = {List.class}, names = {"updatedEntry"})
    ...
    @Getter // glfw events
    @Cancelable
    @ExtraArgs(value = {int.class, int.class, int.class, int.class}, names = {"keyCode", "scanCode", "action", "modifiers"})
    private static final EventChannel<Keyboard> KeyboardInput = new EventChannel<>();
    @Getter
    @Cancelable
    @ExtraArgs(value = {int.class, int.class, int.class}, names = {"eventButton", "action", "mode"})
    private static final EventChannel<Mouse> mouseButton = new EventChannel<>();
    @Getter
    @Cancelable
    @ExtraArgs(value = {double.class, double.class}, names = {"horizontal", "vertical"})
    private static final EventChannel<Mouse> mouseScroll = new EventChannel<>();
    @Getter
    @Cancelable
    @ExtraArgs(value = {double.class, double.class}, names = {"mouseX", "mouseY"})
    ...
    @Getter
    @Cancelable
    @ExtraArgs(value = {int.class, int.class}, names = {"codepoint", "modifiers"})
    ...
    private static final EventChannelDispatcher<EventContainer<?>> customListener = new EventChannelDispatcher<>(EventContainer::getType);
